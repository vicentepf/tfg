...
/**
 * Clase encargada del cambio de contraseña del supervisor
 * 
 * @author Vicente
 *
 */
public class CambiarPassSupervisor extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cambiar_pass_supervisor);
		
		// Recojo la información Extra recibida
        Bundle extras = getIntent().getExtras();
        numEmpleado = extras.getInt("numEmpleado");

		bCambiar = (Button) findViewById(R.id.button_cambiar_pass_supervisor);

		eTpass1 = (EditText) findViewById(R.id.text_cambiar_pass_supervisor_pass1_data);
		eTpass2 = (EditText) findViewById(R.id.text_cambiar_pass_supervisor_pass2_data);
			...
			public void onClick(View view) {
				...
				if (pass1.equals(pass2)){
					estadoCambio = actualizarEmpleado(
							empleado.getNumEmpleado(), pass1, empleado.getNombre(),
							empleado.getApellidos(), empleado.getDireccion(), empleado.getProvincia(),
							empleado.getEmail(), empleado.getTlf(), empleado.getDni(), empleado.getNivel());
					Toast.makeText(getApplicationContext(), estadoCambio, Toast.LENGTH_SHORT).show();
					CambiarPassSupervisor.this.finish();
				} else {
	...
	/**
	 * Consultar un empleado de la base de datos
	 * 
	 * @param numEmpleado
	 * 
	 * @return Empleado
